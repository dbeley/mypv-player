    ...
    case tea.KeyMsg:
        ...
        case "input":
            ...
            case "enter":
                ...
                res := search(query)
                m.list = list.New(toItems(res), list.NewDefaultDelegate(), 0, 0)
            ...
            case "m":
                ...
                m.list = list.New(subs, list.NewDefaultDelegate(), 0, 0)
                m.list.Title = "Subscriptions"
                m.stage = "subs"
            case "q":
                m.list = list.New(toItems(m.queue), list.NewDefaultDelegate(), 0, 0)
                m.list.Title = "Queue"
                m.stage = "queue"
            case "t":
...
func (m model) View() string {
    switch m.stage {
    case "input":
        return fmt.Sprintf("Audio Only: %v\n%s\n\nEnter to search | s latest | m manage subs | q queue | t toggle audio | esc quit", m.audioOnly, m.input.View())
    case "results":
        return m.list.View() + "\n\nenter play | a queue | t toggle audio | esc back"
    case "latest":
        return m.list.View() + "\n\nenter play | a queue | t toggle audio | esc back"
    case "subs":
